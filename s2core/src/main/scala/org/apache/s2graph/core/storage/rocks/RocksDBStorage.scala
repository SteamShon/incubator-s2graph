...
class RocksDBStorage(override val config: Config)(implicit ec: ExecutionContext)
  ...
  override def vertexSerializer(vertex: Vertex) =
    new serde.vertex.VertexSerializable(vertex)

  override val snapshotEdgeDeserializers = Map(
    VERSION1 -> new serde.snapshotedge.tall.SnapshotEdgeDeserializable,
    VERSION2 -> new serde.snapshotedge.tall.SnapshotEdgeDeserializable,
  ...
  override val indexEdgeDeserializers = Map(
    VERSION1 -> new serde.indexedge.tall.IndexEdgeDeserializable(RocksDBHelper.bytesToLong),
    VERSION2 -> new serde.indexedge.tall.IndexEdgeDeserializable(RocksDBHelper.bytesToLong),
    VERSION3 -> new serde.indexedge.tall.IndexEdgeDeserializable(RocksDBHelper.bytesToLong),
    VERSION4 -> new serde.indexedge.tall.IndexEdgeDeserializable(RocksDBHelper.bytesToLong)
  )

  override val vertexDeserializer = new serde.vertex.VertexDeserializable
  ...
  val logDir =
    if (config.hasPath("rocks.storage.log.dir")) config.getString("rocks.storage.log.dir")
    else "./"
  val dbDir =
    if (config.hasPath("rocks.storage.db.dir")) config.getString("rocks.storage.db.dir")
    else "/tmp/rocks"
  ...
  override def fetch(queryRequest: QueryRequest,
    ...
    val queryParam = queryRequest.queryParam
    val edgeWithScores = toEdges(kvs, queryParam, prevStepScore, isInnerCall, parentEdges).filter { case edgeWithScore =>
