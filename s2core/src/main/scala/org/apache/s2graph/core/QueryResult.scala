...
object QueryResult {
  def fromVertices(graph: Graph, query: Query): Seq[QueryRequestWithResult] = {
    if (query.steps.isEmpty || query.steps.head.queryParams.isEmpty) {
      ...
      for {
        ...
        val edge = Edge(graph = graph,
          srcId = vertex.innerId.value,
          tgtId = vertex.innerId.value,
          label = label,
          direction = queryParam.labelWithDir.dir,
          properties = Map(LabelMeta.timestamp.name -> currentTs))
